...
export class DatabaseService {
  ...
  async createUser(user: SignupDto): Promise<string> {
    ...
    newUser.id =
      this.users.length == 0
        ? this.users.length + 1
        : this.users[this.users.length - 1].id + 1; // Simple auto-increment logic
  ...
  async updateUser(
    id: number,
    user: Partial<SignupDto>,
  ): Promise<{ existingUser: User; message: string } | null | string> {
    ...
    if (existingUser) {
      ...
      existingUser.password = newUser.password
        ? ((await this.hashService.hashPassword(newUser.password)) ??
  ...
  async validateUser(
    ...
    const passwordValid = await this.hashService.comparePasswords(
      password,
      user?.password ?? '',
    );
